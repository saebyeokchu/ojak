<!-- find 공지 -->
 <?php 
    ...
    $detailUrl = "/community/detail?gubun=3&pageIndex=".$pageIndex."&id="
 ?>
        ...
        <tbody>
            <?php if(count($pinned) > 0) 
                ...
                <tr>
                    ...
                    <td>
                        <a class="no-text-decoration hover-underline text-dark" href="<?=$detailUrl?><?=$pin->id?>">
                            <?=$pin->title?>
                        </a>
                    </td>
                    <td><?=$pin->user_name?></td>
        ...
        <tbody>
            ...
            <?php if(count($pinned) > 0)
                ...
                <td>
                    ...
                    <td >
                    <a class="no-text-decoration hover-underline text-dark" href="<?=$detailUrl?><?=$pin->id?>"><?=$pin->title?></a></td>
                    <td><?=$pin->view_count?></td>
                </tr>
            <?php } ?>
            <?php if(count($posts) > 0 ) { ?>
            <?php foreach($posts as $post) { ?>
                <tr>
                    <td><?=$post->id?></td>
                    <td >
                    <a class="no-text-decoration hover-underline text-dark" href="<?=$detailUrl?><?=$post->id?>"><?=$post->title?></a></td>
                    <td><?=$post->view_count?></td>
                </tr>
            <?php  } } else { ?>
                <tr>
